...
if ($_SESSION['G-DASH-loggedin'] == true) {
    ...
    if (isRunning($guldenD)) {
        ...
        if ($totalNetworkWeightAdjusted == 0) {
            $totalNetworkWeightAdjusted = $totalNetworkWeight;
        }
        ...
        if ($currentPhase > 1) {
            foreach ($accountlist as $singlewitnessaccount) {
                ...
                if (count($witnessdata) == 1) {
                    ...
                    $checkWOarray = selectElementWithValue($accountlistwitnessonly, "label",
                        $currentwitnessaccountname);
                    ...
                    $witnessdetailsarray['weight_percentage_raw'] = round(($witnessdata['raw_weight'] / $totalNetworkWeight) * 100,
                            2) . "%";
                    $witnessdetailsarray['weight_percentage_adj'] = round(($witnessdata['adjusted_weight'] / $totalNetworkWeight) * 100,
                            2) . "%";
                    $witnessdetailsarray['expected_witness_period'] = $witnessdata['expected_witness_period'];
                    $witnessdetailsarray['estimated_witness_period'] = round($witnessdata['estimated_witness_period'] / 576,
                        2);
                    ...
                    $witnessdetailsarray['lock_until_date'] = date("d/m/Y H:i:s",
                        time() + (($witnessdata['lock_until_block'] - $currentblock) / (576 / (24 * 60 * 60))));
                    $witnessdetailsarray['lock_period'] = $witnessdata['lock_period'];

                    $witnesslockperiod = round(($witnessdata['lock_period'] / 17280), 2);
                    if ($witnesslockperiod == 1) {
                        $witnesslockperiodstring = "Month";
                    } else {
                        $witnesslockperiodstring = "Months";
                    }
                    ...
                    $witnesstransactions_immature = selectElementWithValue($witnesstransactions, "category",
                        "immature");
